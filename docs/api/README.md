...
No ambiente Docker, a porta vem de `APP_PORT` no `.env`. Se `APP_PORT` nao estiver definido, o `docker-compose.yml` publica a API em `8080`.
...
Se voce alterar `APP_PORT`, atualize tambem a variavel `base_url` no environment do Postman.
